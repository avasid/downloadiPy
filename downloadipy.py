...
class Downloader():
    ...
    def path_handler(self, path: str, default: str, url: str, rename: bool = 0) -> tuple:
        ...
        while(not _os.path.isdir(destination)):
            temp_destination = str(input("Destination non-existant:[Current: %s] " % destination))
        ...
        if fname == "" or rename:
            ...
            temp_destination = str(input("Enter destination for downloading:[Current: %s] " % destination))
            temp_fname = str(input("Enter file name for downloading:[Current: %s] " % fname))
            destination = destination if temp_destination is "" else temp_destination
            fname = fname if temp_fname is "" else temp_fname
            destination, fname = self.path_handler(_os.path.join(destination,fname),default,url,0)
        elif _os.path.exists(path):
            choice = str(input("File already exists, [D]ownload | [R]ename | [S]kip(default) :"))
            if choice == "D":
                print("Downloading")
                ...
                rename = 1
                print("Renaming...")
                destination, fname = self.path_handler(_os.path.join(destination,fname),default,url,rename)
            else:
                print("SKIPPING either due to user choice or invalid choice")
                return
        return destination, fname
    
    def request(self, bytesize: int, url: str, method: str = "GET", attempt: int = 0):
        method = self.request_method
        headers = {"Range": "bytes=%d-"%bytesize, "Accept-Encoding": "gzip, deflate, identity, br"}

        self.check_internet()
        try:
            self.content_request = self.session.request(method, url, headers=headers, stream = True, timeout = 10)
        except _requests.exceptions.ConnectionError as e:
            print("Connection Error, checking internet...")
            self.check_internet()
            print("Internet Connected. Retrying download")
            self.download()
            return False    #Because download() starts the whole process again; handling current process with no response.
        ...
        if self.content_request.status_code == 206 or self.content_request.status_code == 200:
            ...
            if attempt < 5:
                ...
                _sys.stdout.flush()
                content_request_status = self.request(bytesize, url, method, attempt+1)
            else:
    ...
    def calculate_remaining_time(total_size: int, downloaded_size: int, speed: int) -> str:
        ...
        if speed != 0:
            try:
                time_remaining = _time.strftime('%H:%M:%S',_time.gmtime((total_size-downloaded_size)//speed))
            except OSError as e:
                if e.errno == 84:
    ...
    def file_handler(self, path: str, content_request, content_size: int, resume: bool = False):
        ...
        open_param, dl = ("ab", _os.path.getsize(path+".mddownload")) if resume else ("wb+", 0)
        chunk_size, content_size_humanized = (1024, None) if content_size is None else (min(int(2*content_size/100),
                                                                                           1048576),
                                                                                       self.humanize_bytes(content_size))
        i = 0
        with open(path+".mddownload",open_param) as fh:
            ...
            try:
                for data in content_request.raw.stream(chunk_size, decode_content = False):
                    ...
                    # time_diff, temp_len_dl, speed, are all there to stabalize the 'speed calculated' when chunk_size is too low.
                    temp_len_dl += len_dl
                    ...
                    if content_size is None:
                        _sys.stdout.write("\rDownloading{}   {}   {}ps".format(("."*i).ljust(4),
                                                                              downloaded_size_humanized.rjust(11),
                                                               self.humanize_bytes(speed).rjust(13)))
                        chunk_size = max(1024,min(int(dl/100), 1048576))
                        i = 0 if i > 3 else i + 1
                    else:
                        done = min(int(50*dl/content_size),50)
                        _sys.stdout.write("\r[{}{}{}]   {}/{}   {}ps   {}".format("=" * done, ">" * bool(50- done),
                                                                            "." * (50 - done - 1),
                                                                            downloaded_size_humanized.rjust(13),
                                                    content_size_humanized if dl<content_size else downloaded_size_humanized,
                                                                    self.humanize_bytes(speed).rjust(13),
                                                                    self.calculate_remaining_time(content_size, dl, speed).ljust(20)))
                    _sys.stdout.flush()
                    time_start = _time.time()
        ...
        self.convert_to_final_file(path+".mddownload", dl, content_request.headers.get("Content-Encoding"))
    ...
    def download(self):
        ...
        if titleheader is None:
            self.title_fetched = _os.path.split(self.url)[-1].split("?")[0].split("#")[0].split("&")[0]
        else:
            # temp_list = titleheader.split("filename=")
            re_obj_fname = _re.search(r'filename=.*?["\'];',titleheader)


            if re_obj_fname is not None:    #len(temp_list)>1
                self.title_fetched = titleheader[re_obj_fname.start()+10:re_obj_fname.end()-2]   #temp_list[-1].strip("'").strip("\"")
            else:
                self.title_fetched = _os.path.split(self.url)[-1].split("?")[0].split("#")[0].split("&")[0]
        ...
        if _os.path.exists(self.path+".mddownload"):
            ...
            if self.filesize is None or self.filesize > self.local_filesize:
                content_request_status = self.request(self.local_filesize, self.url)
                if content_request_status is False:
                    return
                self.is_resume = True
                self.file_handler(self.path,self.content_request,self.filesize,resume=self.is_resume)
            elif self.filesize == self.local_filesize:
                self.convert_to_final_file(self.path+".mddownload", self.filesize, self.content_request.headers.get("Content-Encoding"))
            elif self.filesize < self.local_filesize:
